 ...
 * @retval 操作结果
 */
static unsigned char ft5426_write_byte(unsigned char addr,unsigned char reg, unsigned char data)
{
 ...
 * @retval 读取到的数据。
 */
static unsigned char ft5426_read_byte(unsigned char addr,unsigned char reg)
{
	unsigned char val=0;
 ...
 * @retval None
 */
static void ft5426_read_len(unsigned char addr,unsigned char reg,unsigned char len,unsigned char *buf)
{
	struct i2c_transfer masterXfer;
...
void  ft5426_init (void)
{
	...
	/* 中断IO初始化 */
	ctintpin_config.direction = kGPIO_DigitalIntup;
    ...
    ctintpin_config.interruptMode=kGPIO_NotIntMode;
	...
	lcd_printf("Touch Frimware Version:%#X\r\n", ((unsigned short)reg_value[0] << 8) + reg_value[1]);

	ft5426_dev.initflag = FT5426_INIT_FINISHED;	/* 标记FT5426初始化完成 */
